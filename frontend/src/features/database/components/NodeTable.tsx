            ...
            {data.map((node) => (
              <React.Fragment key={node.id}>
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 text-xs font-outfit font-medium rounded-full ${getImportanceBadge(node.importance)}`}>
                      {node.importance}
                    </span>
                  </td>
                      ...
                      <td colSpan={8} className="px-6 py-4">
                        <div className="space-y-2 text-sm">
                          <div>
                            ...
                            <span className="text-[var(--home-text)]/60 font-outfit font-mono">{node.id}</span>
                          </div>
                          <div>
                            <span className="font-playfair font-medium text-[var(--home-gold-text)]">Tree ID: </span>
                            <span className="text-[var(--home-text)]/60 font-outfit font-mono">{node.tree_id}</span>
                          </div>
                          <div>
                            <span className="font-playfair font-medium text-[var(--home-gold-text)]">Parent ID: </span>
                            <span className="text-[var(--home-text)]/60 font-outfit font-mono">
                              {node.parent_id || '无'}
                            </span>
                          </div>
                          <div>
                            <span className="font-playfair font-medium text-[var(--home-gold-text)]">描述: </span>
                            <span className="text-[var(--home-text)]/60 font-outfit">{node.description}</span>
                          </div>
                          <div>
                            <span className="font-playfair font-medium text-[var(--home-gold-text)]">展开状态: </span>
                            <span className="text-[var(--home-text)]/60 font-outfit">
                              {node.is_expanded ? '已展开' : '未展开'}
                            </span>
                          </div>
                        </div>
                      </td>
